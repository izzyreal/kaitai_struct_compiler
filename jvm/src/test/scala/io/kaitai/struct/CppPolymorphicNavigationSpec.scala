...
class CppPolymorphicNavigationSpec extends AnyFunSuite with Matchers with CppReadWriteSpecSupport {
  ...
  test("parent navigation through polymorphic parents uses virtual base dispatch in runtime") {
    ...
    files("nav_parent_switch_cast.cpp") should include ("m_flag = static_cast<nav_parent_switch_cast_t::foo_t*>(static_cast<kaitai::kstruct*>(_parent())->_parent())->flag();")
  ...
  test("typed parent access is cast at use sites instead of via covariant _parent override") {
    val (specsOpt, problems) = JavaKSYParser.localFileToSpecs("../tests/formats/nav_parent2.ksy", config)
    assertNoNonStyleProblems(problems)
    val compiled = Main.compile(specsOpt.get, specsOpt.get.firstSpec, CppCompiler, config.runtime)
    val files = compiled.files.map(file => file.fileName -> file.contents).toMap

    files("nav_parent2.h") should include ("kaitai::kstruct* _parent() const { return m__parent; }")
    files("nav_parent2.cpp") should include ("m_content = kaitai::kstream::bytes_to_str(m__io->read_bytes(static_cast<nav_parent2_t::tag_t*>(_parent())->num_items()), \"ASCII\");")
  }
}
