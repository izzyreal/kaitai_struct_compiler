...
class CppReadWriteMpc3000AllSpec extends AnyFunSuite with Matchers with CppReadWriteSpecSupport {
  ...
  test("cpp_stl_11 emits a real MPC3000 ALL-like write path") {
    val (specsOpt, problems) = JavaKSYParser.localFileToSpecs("../tests/formats_rw/mpc3000all_write.ksy", config)
    ...
    val header = files("mpc3000all_write.h")
    val source = files("mpc3000all_write.cpp")

    header should include ("class mpc3000all_write_t")
    ...
    source should include ("void mpc3000all_write_t::_write()")
    source should include ("(*it).get()->_write();")
    source should include ("m__io->write_u4le(m_total_number_of_bytes_in_all_sequences);")
  }
}
